...
Proof.
  ...
  - reflexivity.
  - simpl.
    enough (forall (a:nat)(rr:natlist),
               length ([a] ++ rr) = length (rr ++ [a])).
    + rewrite <- (H n (rev l)).
      simpl. rewrite IHl. reflexivity.
    + induction rr.
      * reflexivity.
      * simpl. rewrite <- IHrr. simpl. reflexivity.
Qed.
*)
...
Proof.
  induction l1.
  - simpl. intro. rewrite app_nil_r. reflexivity.
  - simpl. intro. rewrite IHl1.
    enough (forall a b c:natlist, (a++b)++c = a++b++c).
...
Proof.
  ...
  - reflexivity.
  - simpl.
    enough (forall a rr, rev (rr ++ [a]) = a :: rev rr).
    + rewrite H. rewrite IHl. reflexivity.
    + induction rr as [|h tail].
      * reflexivity.
      * simpl. rewrite IHtail. simpl. reflexivity.
Qed.
...
Proof.
  ...
  - simpl. induction l2.
    * simpl. reflexivity.
    * simpl. intros. rewrite IHl2. reflexivity.
...
Proof.
  ...
  - simpl. reflexivity.
  - destruct n.
    + simpl. enough (forall n:nat, n <=? S n = true).
      * rewrite H. reflexivity.
